...
LIST " | <small>*Modified: "+file.mtime+"*</small> | <small>Created: "+file.ctime+"</small>"
...
function toTheProgressBar(share) {
  const progressBarValue = (share * 100).toFixed(1).toString();
  const progressBar = `<progress max="100" value="${progressBarValue}"></progress>`;
  const percentage = (share * 100).toFixed(0).toString() + " %";
  const progressBarWithPercentage = `${progressBar} ${percentage}`;
